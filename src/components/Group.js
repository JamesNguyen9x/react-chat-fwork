...
export default class Group extends Component {
  ...
  componentWillMount() {
    if ( this.props.groupInfo ) {
      ...
      console.log('this.props.groupInfo', this.props.groupInfo.members)
      ...
      fetchAPI(baseURL, url, 'GET').then(res => {
        this.setState({
          ...
          this.setState({
            nameGroup: name,
            usersChecked: members.map(member => member.userInfo),
  ...
  onCreateGroup = () => {
    ...
    let data = {
      users : usersChecked.map(user => user._id),
    ...
    this.props._createGroup(data).then(response => {
      ...
      if (response && response.status === 200) {
        ...
        this.props.handleCancer();
      }
    }).catch(err => {
      console.log('err _createGroup: ', err)
    })
  }
  ...
  onEditGroup = () => {
    ...
    // let data = {
    //   roomId: this.props.dataEdit._id,
    //   userIds : usersChecked.map(user => user._id),
    //   company: companyPicked,
    //   name : nameGroup,
    //   avatarUrl: avatarUrl
    // }

    // this.props._editGroup(data).then(response => {
    //   this.setState({
    //     usersChecked: [],
    //     nameGroup: ''
    //   })
    ...
    //   this.props.handleCancer()
    // }).catch(err => {
    //   console.log('err _editGroup: ', err)
    // })
  }
        ...
        <div className="create-group-box">
          <div className="content">
            <div className="header">
                ...
                <p style={{cursor: 'pointer'}} onClick={this.props.handleCancer} className="closeIcon">X</p>
            </div>
            ...
            <div className="footer">
              <button className="btn-cancer" onClick={this.props.handleCancer}>Hủy</button>
              {edit ?
